 ...
 Business Agility : 업무변경량, 추가/변경 csr건수 , 확장가능성  , 현업인터뷰(비즈니스변화민첩대응,신기술도입유연성확보)
 장애대응 : 트랜잭션발생량,(트래픽폭증으로 안정적시스템구축)
 서비스독립성 : 역공학 호출관계, crud,(서비스재정의를 통해 분산아키텍처에 복잡성이 높아지는 문제해결)
 배포 용이성 : 현행시스템 기준 측정
...
. 예약내역생성취소 :병원메인 시스템의 고객업무에서 유효예약이용권 보유여부에 따라 예약이용권을 차감하거나, 구매요청알림을 등록하도록 하는 통합 api를 제공을 요청함. 또한, 원격진료예약시, 예약내역생성을 보상가능 트랜잭션으로 병원메인 시스템의
 고객업무에서 제공하는 통합 api를 피봇트랜잭션으로 처리하도록 호출순서 변경하면 원격진료 서비스의 보상서비스만으로 업무처리 가능함
. 배송취소/결제정보수정취소 : 배송정보등록 및 배송지시를 한버네 처리하는 api로 제공받고, 약제배송및결제서비스이 api를 보상가능트랜잭션으로, 힐링캐쉬차감을 피봇트랜잭션으로 예약상태변경을 재시도가능트랜잭션으로 처리하면 병원 메인 시스템의보상
 트랜젝션추가개발필요없음
 
 ##############
 srp 
 ocp : 새로운 기능으로 변경시에 기존 코드의 냉요을 수정하는 것은 확장에 열려있고 수정에 닫혀있어야하는 OCP원칭게어긋함
 lsp
 dip
 isp
 command패턴은 요청을 객체화및 캡슐화하여 사용자가 보낸 요청을 나중에 이용할수있도록 메서드이름,매개변수등 요청에 필요한 정보를 이용할수있는 패턴이다. 
 중재자패턴은 각 컴포넌트간 연결을 느슨하게하고 상호작용을 도맡아서...처리하는 객체를 두는 경우..중재자..
 전략패턴 알고리즘이 클래스별로 캡슐화되어있고..필요할때 교체할수있는 동일한문제를 다른 알고리즘으로제공가능하다. 전략패턴
 이벤트청구와 i청구를 연결하는 아답터가 필요하다 아답터패턴
 옵저버패턴은 데이터의 변경이 발생했을때 상대클래스나 객체에의존하지 않고 데이터 변경을 통보하는 방시기다. 
 
 Orchestration Choreography
장점
• 기존 설계방식과 유사하게 Logic을 중앙에서
컨트롤 하여 이해하기 쉬움
• 구현과 테스트가 상대적으로 쉬움
• 순환 종속성(Cyclic Dependencies) 감소
• Event 방식으로 Saga Pattern에 충실한 방법
• 참여하는 서비스 간의 결합도가 낮음
(Loosely Coupled)
• Event/Message 수신 시 각자의 역할에만 충실,
타 서비스들의 처리 로직을 알 필요가 없음
단점
• 중앙 통제하는 Service/API에 복잡하거나
많은 로직이 포함될 수 있음
• Orchestrator 역할의 추가 서비스가 필요할 수 있음
• 호출되는 Service/API들의 입출력 및 처리순서로
인한 결합도가 높음
• 작업처리가 서비스 별 분산 구현되어
(Decentralized Implementation)
Orchestration 방식에 비해 이해하기 어려움
• Event를 주고받는 서비스들 간의 순환 종속성 증가
(Cyclic Dependencies)
• 이벤트 간 추적/Debugging이 어려울 수 있고
처리 Step이 추가/변경될 때 혼란스러울 수 있음
 
 데이터를 복제하여 대량 자료 조회에 적합한 방식
• 변경 후 데이터를 즉시 조회할 수 없으며 특정 시간 안에만 동기화 해도 문제가 없을 때 사용
• MSA에서는 Message Queue를 많이 채택하고 있으며, 업무와 프로젝트에 알맞은 방식을 선택해야 함
CDC
(Changed Data
Capture)
Batch/ETL
Message Queue
• 비교적 손쉽게 활용할 수 있음
• DB 또는 CDC 솔루션 특성이 존재하므로
선택 적용
• 주기적으로 대량 데이터 복제 가능
• 복제 주기가 비교적 길어
적시성이 높은 업무 사용 불가
• 비동기 방식으로 데이터 복제 가능
• Message Queue를 이용하지 않고
단방향 Notification Asynch로도
데이터 전달 가능
• MSA 환경에서 데이터 공유 설계 시 많이 사용
 
 sharedDB       
 remote DB(DBLink) 권장x
 CDC/replica cdc별솔루션특성
 Message Queue 적시성어려움
 Batch/ETL 즉시성x
 Cache 변경빈번x
 Api연계 대량조회x
 ##########
 ...
 회사와 인터페이스id를 이용하여 회사별전문으로 호출하고 호출된전문항목을 매핑하여 형을 변환 후 서비스에 사용하는 dto리턴
 구현매커니즘
 (1) 업무에서 각 회사기간계를 호출하는 서비스에서사용해야하는 dto와 회사정보, 인터페이스ID를 전달하여 executeServiceInfo()를호출한다. 
  (2) executeServiceInfo에서는 회사와 인터페이스ID를 통하여 각 회사의 각ㄴ계 서비스의 매핑 정보를 이용하여 기간계 서비스를 호출한다. 
 (3) 호출된 ㄱ간계 서비스이ㅢ 리턴타입과 Saas솔루션에서 사용하는 Dto데이터의 형과 매핑정보를 읽어 형을 변화하여 dto정보로 변환
 (4) 변환된 dto정보르르 호출된 서비스로 반환한다.
 . saas솔루션의 인터페이스정보 : 회사별 인터페이스 정보와 호출방식, target/source정보를 가지고있다.
 .인터페이스기능과 기간계 서비스 매핑정보 : saas솔루션의 서비스에서불러야 하는 각 회사의 기간계 서비스전문정보
 .인터페이스 dto와 기간계 서비스의전문정보 매핑 : dto와 기간계 서비스린턴 정보의 매핑 규칙정의
